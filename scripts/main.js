...
function initLoader() {
    ...
    const progressEl = document.querySelector("[data-loader-progress]");
    if (!loader || !lineEl || !progressEl)
        return;
...
function initCursor() {
    ...
    if (!cursor || window.matchMedia("(max-width: 700px)").matches)
        return;
    let cx = -100, cy = -100;
    let tx = -100, ty = -100;
...
function initHeader() {
    ...
    if (!header)
        return;
    const obs = new IntersectionObserver(([entry]) => header.classList.toggle("is-scrolled", !entry.isIntersecting), { rootMargin: "-1px 0px 0px 0px", threshold: 0 });
...
function initCounters() {
    const elements = Array.from(document.querySelectorAll("[data-count-to]"));
    if (!elements.length)
        return;
    const obs = new IntersectionObserver(entries => {
        entries.forEach(e => {
...
function initCaseTabs() {
    ...
    buttons.forEach(btn => {
        btn.addEventListener("click", () => {
            ...
            const panel = document.querySelector(`[data-case-panel="${target}"]`);
...
function initTerminal() {
    ...
    function appendOutput(text, cls = "out") {
    ...
    document.querySelectorAll("[data-copy]").forEach(btn => {
        btn.addEventListener("click", async () => {
            ...
            try {
                ...
                if (toastEl)
                    toastEl.textContent = "Copy failed — please copy manually.";
            }
        });
    });
}
